...
function createSymbol() {
  console.log(Symbol()); // Повернемо символ без опису
}
...
function createSymbolWithDescription(property) {
 ...
 console.log(symbol); //Повертаємо опис символа symbol
}
...
function createAndCompareTwoSymbols(value) {
  ...
  console.log(symbol1 === symbol2); // Створюємо два символи з однаковим описом value, та повертаємо результат порівняння двох символів
}
...
function createSymbolForKey(key) {
  let symbol1 = Symbol.for(key); // Використовуємо Symbol.for() для створення нового символу symbol зі значенням key
...
function useSymbolToStringTag() {
  ...
  const SymbolKey = Symbol("CustomObject");
  myObject[SymbolKey] = "CustomObject"; // Встановлюємо властивість Symbol об'єкту myObject на значення "CustomObject".
  return myObject[SymbolKey].toString(); // Повертаємо рядкове представлення myObject[Symbol].
}
...
let rangeObject = {
  from: 1, // Використовуємо Symbol.iterator для створення ітератора всередині об'єкта "myObject"
  to: 7, // this.current присвоюємо this.from
  [Symbol.iterator]() { 
...
function useSymbolIterator(obj) {
  let result = []; 
  for (let value of rangeObject) { // Проходимо крізь елементи об'єкта obj, використовуючи цикл "for...of"
    result.push(value); // Додаємо кожне значення до масиву "result"
  }
...
let myObject = {
  ...
  next() { // Оголошення методу "next" для ітерації
    if (this.currentIndex < this.category.length) { // Створюємо логічний оператор який буде перевіряти чи властивість об'єкту currentIndex менша ніж довжина масиву category
      const value = this.category[this.currentIndex].name; //Створюємо змінну value якій присвоємо властивість name елемента масиву category з індексом currentIndex
